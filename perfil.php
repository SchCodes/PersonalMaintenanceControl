...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($validacao === true) {
            $foto_blob = imagemParaBlob($_FILES['foto']);
        } else {
    ...
    if (!empty($nova_senha)) {
        if (!password_verify($senha_atual, $usuario['senha'])) {
            ...
            $stmt = executarQuery($sql_update, [$nome, $email, $cargo, $hash_senha, $foto_blob, $usuario_id]);
            if ($stmt->rowCount() > 0) {
                $_SESSION['nome'] = $nome;
        ...
        $stmt = executarQuery($sql_update, [$nome, $email, $cargo, $foto_blob, $usuario_id]);
        if ($stmt->rowCount() > 0) {
            $_SESSION['nome'] = $nome;
            $mensagem = '<div class="alert alert-success">Perfil atualizado com sucesso!</div>';
            $usuario = executarQuery($sql, [$usuario_id])->fetch();
